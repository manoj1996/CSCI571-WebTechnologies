...
import {Modal, Card, Button, Badge, Image} from 'react-bootstrap';
...
class Home extends Component
{
    ...
    callGuardian()
    {
        fetch("https://news-api.azurewebsites.net/guardian/home").then(res=> {
            return res.json();
        }).then(news_articles=> this.setState({articles:news_articles}));
    }
    callNyTimes()
    {
        fetch("https://news-api.azurewebsites.net/nyTimes/home").then(res=> {
            return res.json();
    ...
    returnBadge(section)
    {
        ...
        return (
            <Badge style={{float:'right',backgroundColor:color, color:textColor, marginTop:'0.2%'}}>
                {section.toUpperCase()}
            </Badge>
        )
    }
                ...
                <div style={{marginBottom:'3%'}}>
                {
                    articlesArray.map((article) =>
                        <Container fluid={true} className="card-container">
                                <Card style={{boxShadow:'0px 0px 25px #333',marginTop:'2%',width:'100%', cursor:'pointer'}} onClick={(event) => this.callDetailed(event,article)}>
                                    <Row xs={1} md={1} lg={1} xl={1} sm={1}>
                                    <Card.Body>
                                        <Col xl={3} xs={12} md={3} lg={3} sm={3}>
                                            <Card.Img src={article.Image} height={'auto'} width={'auto'} className="card-img"/>
                                        </Col>
                                        <Col xl={9} xs={12} sm={9} lg={9} md={9} style={{float:'right'}}>
                                                ...
                                                <span>
                                                    <TextTruncate line={3}  style={{fontSize:'98%'}} text= {article.Description} />
                                                </span>
